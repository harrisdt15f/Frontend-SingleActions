    ...
    public function execute(FrontendApiMainController $contll, $inputDatas): JsonResponse
    {
        if ($inputDatas['type'] == $this->stopAllTraceType) {
            $traceListsEloqs = $this->model->getUnfinishedTrace(
                $inputDatas['lottery_traces_id'],
                $contll->partnerUser->id
            );
        ...
        if ($inputDatas['type'] == $this->stopAllTraceType) {
            $finishedTraceListsEloqs = $this->model->getFinishedTraceToRun(
                $inputDatas['lottery_traces_id'],
                $contll->partnerUser->id
            );
            //有半状态正在运行的状态情况下，如果有不对lotteryTrace 做任何改变
            if ($finishedTraceListsEloqs->isEmpty()) {
                $finishedTraceListsEloqs = $this->model->getFinishedTrace(
                    $inputDatas['lottery_traces_id'],
                    $contll->partnerUser->id
                );
            ...
            //有半状态正在运行的状态情况下，如果有不对lotteryTrace 做任何改变
            $runningFlag = $this->model->getRuningTrace(
                $inputDatas['lottery_trace_lists_id'],
                $contll->partnerUser->id
            );
            if ($runningFlag->isEmpty()) {
                $unFinishedTraceListsEloqs = $this->model->getUnfinishedTraceAllWating(
                    $inputDatas['lottery_trace_lists_id'],
                    $contll->partnerUser->id
                );
                //如果是空代表执行结束  否则状态不变
                ...
                $lotteryList = $this->model->getUnfinishedTraceAll(
                    $inputDatas['lottery_trace_lists_id'],
                    $contll->partnerUser->id
                );
